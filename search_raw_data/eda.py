...
def main():
    print("[시작] 의결서 분류 작업 시작 (500개 대상)...")
    results = process_files(limit=500)
    
    # CSV 저장
    os.makedirs(os.path.dirname(OUTPUT_FILE), exist_ok=True)
    with open(OUTPUT_FILE, 'w', encoding='utf-8-sig', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=["의결서번호", "의결서제목", "위반분야", "기업규모", "법적지위", "업종"])
        writer.writeheader()
        writer.writerows(results)
    
    print(f"[완료] 분류 완료! 결과 파일: {OUTPUT_FILE}")
...
def run_visualization():
    if not os.path.exists(OUTPUT_FILE):
        print(f"[오류] 시각화할 파일 '{OUTPUT_FILE}'이 없습니다.")
        return

    print(f"[시각화] 심화 분석 및 시각화 시작...")
    ...
    axes = axes.flatten()
    target_cols = ["위반분야", "기업규모", "법적지위", "업종"]
    
    for i, col in enumerate(target_cols):
        counts = df[col].value_counts()
        sns.barplot(x=counts.values, y=counts.index, ax=axes[i], palette="viridis", hue=counts.index, legend=False)
        axes[i].set_title(f"[{col}] 기초 분포", fontsize=16, fontweight='bold')
        for j, v in enumerate(counts.values):
            axes[i].text(v + 0.5, j, str(v), va='center')
    
    plt.suptitle("공정위 의결서 기초 데이터 분포 (법률 용어 적용)", fontsize=22, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(OUTPUT_FILE.replace('.csv', '_base.png'), dpi=300)
    plt.close()

    # 2. 심화 분석 - 히트맵 (업종 x 위반분야)
    plt.figure(figsize=(14, 10))
    cross_tab = pd.crosstab(df['업종'], df['위반분야'])
    sns.heatmap(cross_tab, annot=True, fmt='d', cmap="YlGnBu", cbar=True)
    plt.title("산업별 위반 유형 집중도 히트맵", fontsize=18, fontweight='bold', pad=20)
    plt.xlabel("위반 분야", fontsize=12)
    plt.ylabel("산업 군", fontsize=12)
    plt.savefig(OUTPUT_FILE.replace('.csv', '_heatmap.png'), dpi=300, bbox_inches='tight')
    plt.close()

    # 3. 심화 분석 - 법적 지위별 위반 행태 분석
    plt.figure(figsize=(14, 10))
    role_violation = pd.crosstab(df['법적지위'], df['위반분야'])
    role_violation.plot(kind='barh', stacked=True, figsize=(14, 8), colormap="Set3")
    plt.title("법적 지위(공식 용어)별 주요 위반 행태", fontsize=18, fontweight='bold', pad=20)
    plt.xlabel("건수", fontsize=12)
    plt.ylabel("법적 지위", fontsize=12)
    plt.legend(title="위반 분야", bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(OUTPUT_FILE.replace('.csv', '_role_cross.png'), dpi=300, bbox_inches='tight')
    plt.close()

    # 4. 기업규모 x 법적지위 교차 분석
    plt.figure(figsize=(14, 10))
    size_role = pd.crosstab(df['기업규모'], df['법적지위'])
    sns.heatmap(size_role, annot=True, fmt='d', cmap="Reds")
    plt.title("기업규모와 법적 지위의 상관관계 (갑을관계 실체 분석)", fontsize=18, fontweight='bold', pad=20)
    plt.xlabel("법적 지위", fontsize=12)
    plt.ylabel("기업 규모", fontsize=12)
    plt.savefig(OUTPUT_FILE.replace('.csv', '_size_role.png'), dpi=300, bbox_inches='tight')
    plt.close()

    print(f"[완료] 심화 시각화 완료! (Base, Heatmap, Role_Cross, Size_Role 4종 생성)")
