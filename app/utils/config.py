...
def resolve_reranker_top_n() -> int:
    return _get_env_int("FAIRDATA_RERANK_TOP_N", 150)
...
# fusion 단계에서 multivector 경로 가중치를 반환합니다.
def resolve_multivector_weight() -> float:
    return _get_env_float("FAIRDATA_MULTIVECTOR_WEIGHT", 1.05)


# Q-M 계열 복합질문에만 multivector 경로를 적용할지 반환합니다.
def use_multivector_only_for_qm() -> bool:
    return _get_env_bool("FAIRDATA_MULTIVECTOR_QM_ONLY", False)
...
def is_llm_trust_remote_code_enabled() -> bool:
    return os.getenv("FAIRDATA_LLM_TRUST_REMOTE_CODE", "0").strip().lower() in ("1", "true", "yes")
