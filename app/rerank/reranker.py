...
class TransformerSequenceClassificationReranker:
    ...
    def build_pair_text(self, analysis: QueryAnalysis, chunk_id: str):
        chunk = self.corpus_store.chunk_map.get(chunk_id)
        if chunk is None:
            return None
        parts = [chunk.content]
        if chunk.header or chunk.section:
            context = " > ".join(filter(None, [chunk.header, chunk.section]))
            parts.insert(0, f"[{context}]")  # 짧게, 앞에 붙임
    ...
    #     for chunk_id in valid_ids:  # fused_scores 전체 X, rerank된 것만
    #         chunk = self.corpus_store.chunk_map.get(chunk_id)
    #         if chunk is None:
    #             continue
    ...
    #         reason_scores = {
    #             "fusion": fused_score,
    #             "reranker": rerank_map[chunk_id] * self.rerank_weight,
    ...
    def rerank_chunks(
        ...
        *,
        ...
        ranked_chunks: list[RankedChunk] = []
        for chunk_id in valid_ids:  # ← fused_scores.items() 대신 valid_ids로 제한
            chunk = self.corpus_store.chunk_map.get(chunk_id)
            if chunk is None:
                continue
            document = self.corpus_store.document_map[chunk.doc_id]
            fused_score = fused_scores.get(chunk_id, 0.0)  # ← fused_score 별도로 가져오기
            reason_scores = {
                "fusion": fused_score,
                "reranker": rerank_map[chunk_id] * self.rerank_weight,  # ← .get() 불필요, 항상 존재
                "entity": self.entity_boost(analysis, document),
            }
            final_score = sum(reason_scores.values())
            reasons = tuple(name for name, score in reason_scores.items() if score > 0)
