...
_DENSE_BACKEND_ALIASES: dict[str, dict[str, Any]] = {
    ...
    "gte_multilingual": {"query_prefix": "", "document_prefix": "", "trust_remote_code": True},
...
class STModelRuntime:
    # sentence-transformers 기반 dense 모델을 래핑합니다.
    def __init__(
        self,
        model_dir: Path,
        *,
        ...
        requested_device = os.getenv("FAIRDATA_DENSE_DEVICE", "cuda" if torch.cuda.is_available() else "cpu").strip().lower()
        if requested_device.startswith("cuda") and not torch.cuda.is_available():
            requested_device = "cpu"
        self.device = requested_device
        self.default_task = "retrieval" if backend_name == "jina_v4" else None
        model_kwargs = {"default_task": self.default_task} if self.default_task else None
    ...
    def _encode_kwargs(self) -> dict[str, Any]:
        ...
        batch_size = int(os.getenv("FAIRDATA_ST_BATCH_SIZE", os.getenv("FAIRDATA_EMBED_BATCH_SIZE", "8")))
        kwargs["batch_size"] = max(1, batch_size)
        if self.default_task:
            kwargs["task"] = self.default_task
        return kwargs

    def encode_documents(self, texts: list[str]) -> np.ndarray:
        inputs = [f"{self.document_prefix}{text}" for text in texts]
        matrix = self.model.encode(inputs, **self._encode_kwargs())
        return np.asarray(matrix, dtype="float32")

    def encode_query(self, question: str) -> list[float]:
        query_inputs = [f"{self.query_prefix}{question}"]
        matrix = self.model.encode(query_inputs, **self._encode_kwargs())
        if len(matrix) == 0:
            return []
