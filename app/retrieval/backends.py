...
class STModelRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self.default_task = "retrieval" if model_dir.name == "jina-embeddings-v4" else None
        self.default_batch_size = int(os.getenv("FAIRDATA_ST_BATCH_SIZE", os.getenv("FAIRDATA_EMBED_BATCH_SIZE", "32")))
    ...
    def encode_documents(self, texts: list[str]) -> np.ndarray:
        ...
        encode_kwargs = {"normalize_embeddings": True, "batch_size": self.default_batch_size}
        if self.default_task:
            encode_kwargs["task"] = self.default_task
        matrix = self.model.encode(inputs, **encode_kwargs)
    ...
    def encode_query(self, question: str) -> list[float]:
        ...
        if self.default_task:
            encode_kwargs["task"] = self.default_task
        matrix = self.model.encode(query_inputs, **encode_kwargs)
        if len(matrix) == 0:
            return []
