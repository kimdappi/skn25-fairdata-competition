...
export FAIRDATA_LLM_MODEL_DIR=./models/llm/exaone-3.5-7.8b-instruct
...
run_python_u scripts/build_route_tags.py \
  --data-dir ./data/raw \
...
run_python_u scripts/build_indexes.py | tee "$RESULTS_DIR/build.log"
nohup "${UVICORN_RUNNER[@]}" server:app --host 0.0.0.0 --port ${PORT} > "$LOG" 2>&1 &
SERVER_PID=$!
...
run_python_u scripts/evaluate_local.py \
  --eval-file ./data/test/eval_dataset_260505.json \
