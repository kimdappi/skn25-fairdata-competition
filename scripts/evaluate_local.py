...
def parse_int_list_arg(text: str | None, default: list[int]) -> list[int]:
    if text is None:
        values = list(default)
    else:
        ...
        if not raw_items:
            return list(default)
        values = [int(item) for item in raw_items]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--recall-ks",
        default="5",
        help="쉼표 구분 Recall@K 목록. 예: 5,10,20,30,40,50",
    )
    parser.add_argument(
        "--mrr-ks",
        default="5",
        help="쉼표 구분 MRR@K 목록. 예: 5,10,20,30,40,50",
    )
...
def main() -> None:
    args = parse_args()
    recall_ks = parse_int_list_arg(args.recall_ks, default=[5])
    mrr_ks = parse_int_list_arg(args.mrr_ks, default=[5])
        ...
        rows.append(
            {
                ...
                "mrr": mrr,
                "token_f1": token_f1,
                "recall_metrics": compute_recall_metrics(predicted_chunk_ids, example.gold_chunk_ids, recall_ks),
                "mrr_metrics": compute_mrr_metrics(predicted_chunk_ids, example.gold_chunk_ids, mrr_ks),
            }
        )
        print(
